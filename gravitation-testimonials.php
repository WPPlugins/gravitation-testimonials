...
function gravitation_testimonials_stylesheet() {
	...
	$check_gravitation_bootstrap = get_option('gravitation_bootstrap');
	    if($check_gravitation_bootstrap == 1):
			$handle = 'bootstrap.min.css';
			if (wp_style_is( $handle, $list = 'enqueued, to_do, registered' )) {
				return;
	     	} else {
		 		wp_enqueue_style( 'gravitation_gravitations_bootstrap', plugins_url( 'bootstrap/css/bootstrap.min.css', __FILE__ ), array(), '3.3.5', true  );
	    ...
	    if($check_gravitation_bootstrap == 1):
			...
			if (wp_script_is( $handle, $list )) {
				return;
		 		 ...
		 		 wp_register_script('gravitation_gravitations_bootstrap_js', plugin_dir_url( __FILE__ ).'/bootstrap/js/bootstrap.min.js');
		 		 wp_enqueue_script('gravitation_gravitations_bootstrap_js', array('jquery'), '3.3.5', true );
	     	}
     	endif;
...
function gravitation_testimonials_help_page() { 
	...
	if(isset($_REQUEST['update_gravitation_testimonials_settings'])){ 
				if ( !isset($_POST['gravitation_testimonials_nonce']) || !wp_verify_nonce($_POST['gravitation_testimonials_nonce'],'gravitation_testimonials_settings') ){
				    _e('Sorry, your nonce did not verify.', 'gravitation_testimonials');
				   exit;
				}else{
					
				  	update_option('gravitation_bootstrap',$_POST['gravitation_bootstrap']);
		...
		<div id="custom-branding-general" class="wrap">
			...
			<div class="metabox-holder">
				...
				<div class="inside">
					...
					<form method="post" action="edit.php?post_type=testimonials&page=gravitation-testimonials.php">
					<?php settings_fields( 'gravitation-testimonials-settings-group' ); ?>
					<?php do_settings_sections( 'gravitation-testimonials-settings-group' ); ?>
				    <table class="form-table">
				        <tr valign="top">
				        <th scope="row"><?php _e('Include Bootstrap?','gravitation_testimonials'); ?>
					        <div class="sidebar-description">
				<p class="description"><?php _e('If your theme already includes bootstrap set this option to NO.','gravitation_testimonials'); ?></p>
			</div></th>
				        
				        <td>
				        <select name="gravitation_bootstrap">
							<?php
								$check_gravitation_bootstrap = get_option('gravitation_bootstrap');
								for($i=0;$i<2;$i++){
									if($i == 0){
										$yes_no = __('No','gravitation_testimonials');
									}else{
										$yes_no = __('Yes','gravitation_testimonials');
									}
									echo '<option value="'.$i.'"'.selected($check_gravitation_bootstrap, $i, false).'>'.$yes_no.'</option>';	 
								}		 
							?>										
						</select>
				        </td>
					...
					<?php wp_nonce_field( 'gravitation_testimonials_settings', 'gravitation_testimonials_nonce' ); ?>
				    <p class="submit">
				        <input class="button-primary" type="submit" name="update_gravitation_testimonials_settings" value="<?php _e( 'Save Settings', 'gravitation_testimonials' ) ?>" />
				    </p> 
